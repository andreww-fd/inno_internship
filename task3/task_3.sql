...
select c.name, count(1) as num_of_films from film
...
select a.actor_id, concat(concat(a.first_name, ' '), a.last_name) as artist_name, sum(f.rental_duration) as total_rental_duration 
from film f
...
with t1 as (select a.actor_id, concat(concat(a.first_name, ' '), a.last_name) as artist_name, count(*) as num_of_films,
...
with t1 as (select city.city, category.name, sum(date_part('day', return_date - rental_date)) as total_rental_days from film
	...
	order by city.city, total_rental_days desc)
select * from t1
where total_rental_days = (select max(total_rental_days) from t1 t where t.city = t1.city group by city);
...
t3 as (select t2.city, t1.name, sum(date_part('day', t2.return_date - t2.rental_date)) as total_rental_days from t1
	join t2 on t1.inventory_id = t2.inventory_id
	group by t2.city, t1.name
	order by t2.city)

select * from t3
where total_rental_days = (select max(total_rental_days) from t3 t where t.city = t3.city group by city);
